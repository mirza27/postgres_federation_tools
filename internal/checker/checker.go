...
type PivotStore interface {
	FetchForChecker(ctx context.Context, limit int) ([]pivot.CheckerItem, error)
	MarkReady(ctx context.Context, id int64) error
	MarkError(ctx context.Context, id int64, msg string) error
	EnsureKeymapRequest(ctx context.Context, queueID uuid.UUID, km pivot.KeymapRequest) (int64, error)
	AttachKeymap(ctx context.Context, id int64, keymapID int64, status string) error
}

func New(pivotRepo PivotStore, plan *mapping.Planner, maxRows, intervalMs int) *Checker {
	return &Checker{
		Pivot:    pivotRepo,
		Plan:     plan,
		MaxRows:  maxRows,
